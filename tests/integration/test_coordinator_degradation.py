...
def test_coordinator_fails_when_critic_fails(tmp_path) -> None:
    ...
    with pytest.raises(RuntimeError, match="critic unavailable"):
        coordinator.run_topic("protein binder design")

    # Verify DB state reflects failure at both run and stage level
    run_summary = repo.list_runs()[0]
    assert run_summary.status == "failed"
    result = repo.build_final_result(run_summary.run_id)
    summaries = {s.stage_name: s for s in result.stage_summaries}
    assert summaries["retrieval"].status == "completed"
    assert summaries["review"].status == "completed"
    assert summaries["critic"].status == "failed"
    assert summaries["critic"].error_message == "critic unavailable"
...
def test_coordinator_can_rerun_planner_after_failure(tmp_path) -> None:
    ...
    rerun_result = coordinator.rerun_planner(run_id)
